...
countCond <- read.csv("FEBA/countCond.csv", header = TRUE, row.names = 1, sep = ",") 
countT0 <- read.csv("FEBA/countT0.csv", header = TRUE, row.names = 1, sep = ",")

# Load metadata describing experimental conditions and replicates.
expsUsed <- read.csv("FEBA/expsUsed.csv", header = TRUE, row.names = 1, sep = ",")
...
normalize_fitness <- function(fitness, genes, minToUse = 5) {
...
minTotalCounts <- 10   # Minimum total counts across all conditions.
minNonZeroConditions <- 10  # Minimum number of conditions with at least one count.
...
names(fitness_results) <- colnames(countT0)  # Name results by experiment.

# Combine and summarize fitness results across all experiments.
all_fitness <- do.call(cbind, lapply(fitness_results, function(df) df$fit))  # Merge fitness results.
aggregated_results <- aggregate_fitness(all_fitness, filtered_genes)  # Summarize results.
...
gene_info <- read.csv("FEBA/gene_information.csv", header = TRUE, sep = ",")
...
top_negative_fit <- aggregated_results %>%
  arrange(fit) %>%  
  slice_head(n = 20)  

# Create a bar plot to visualize these fitness values.
library(ggplot2)
ggplot(top_negative_fit, aes(x = reorder(product, -fit), y = fit)) +  
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +  
  labs(
    title = "Most Negative Fitness Values (All conditions)",  
