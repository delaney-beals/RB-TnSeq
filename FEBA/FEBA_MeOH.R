...
genes <- read.csv("FEBA/MeOH/genes.csv", header = TRUE, sep = ",")
...
countCond <- read.csv("FEBA/MeOH/countCond.csv", header = TRUE, row.names = 1, sep = ",") 
countT0 <- read.csv("FEBA/MeOH/countT0.csv", header = TRUE, row.names = 1, sep = ",")

# Load metadata describing experimental conditions and replicates.
expsUsed <- read.csv("FEBA/MeOH/expsUsed.csv", header = TRUE, row.names = 1, sep = ",")
...
all_fitness_MeOH <- do.call(cbind, lapply(fitness_results, function(df) df$fit))  # Merge fitness results.
aggregated_results_MeOH <- aggregate_fitness(all_fitness_MeOH, filtered_genes)  # Summarize results.
...
aggregated_results_MeOH <- merge(
  aggregated_results_MeOH,  
...
top_negative_fit_MeOH <- aggregated_results_MeOH %>%
  arrange(fit) %>%  
  slice_head(n = 20)  

# Create a bar plot to visualize these fitness values.
library(ggplot2)
ggplot(top_negative_fit_MeOH, aes(x = reorder(product, -fit), y = fit)) +  
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +  
  labs(
    title = "Most Negative Fitness Values (MeOH conditions)",  
